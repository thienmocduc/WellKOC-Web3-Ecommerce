...
@router.put("/profile")
async def update_koc_profile(body: KOCProfileUpdate, current_user: CurrentUser = Depends(require_role([UserRole.KOC])), db: AsyncSession = Depends(get_db)):
...
async def generate_affiliate_link(product_id: UUID, current_user: CurrentUser = Depends(require_role([UserRole.KOC]))):
    import hashlib
...
async def koc_analytics(current_user: CurrentUser = Depends(require_role([UserRole.KOC])), db: AsyncSession = Depends(get_db)):
